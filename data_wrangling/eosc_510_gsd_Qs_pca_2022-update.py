...
# Removing helpyr dependency
import logger # copied into local repo
import figure_helpyr # copied into local repo
import data_loading  # copied into local repo
#from helpyr import logger
#from helpyr import data_loading
#from helpyr import kwarg_checker
#from helpyr import figure_helpyr

# Removing use of omnipickle manager
...
class EOSC510Project:
    ...
    def __init__(self, debug_mode=False):
        ...
        # Start up logger
        self.log_filepath = f"{settings.log_dir}/eosc510_project_2022-update.txt"
        ...
        self.experiment_keys = ['1A', '1B', '2A', '2B', '3A', '3B', '4A', '5A']
        rising_steps = [('rising', str(d)) for d in [50, 62, 75, 87, 100]]
        falling_steps = [('falling', str(d)) for d in [87, 75, 62]]
        self.steps = rising_steps + falling_steps
        self.valid_times = [str(t)+'0' for t in range(0,7)]
        self.feed = settings.feed_rates
    ...
    def load_data_files(self):
        ...
        # Read the light table data from all the separate files representing 
        # each period. The original version (using omnimanager) stored all data 
        # types (lighttable, sieve, etc.) from all experiments (1A, 1B, 2A, 
        # etc.) and could return any combination of data or times.
        period_frames = self.read_period_files()
        Qs_data = self.assemble_frames(period_frames)

        # Temporarily read from pickle instead of txt files for speed
        # Helpful if you will be running this script a lot for debugging
        #temp_pickle_path = pjoin(settings.pca_pickle_dir, 'data_dict_pickle.pkl')
        #self._write_pickle(Qs_data, temp_pickle_path)
        #Qs_data = self._load_pickle(temp_pickle_path)

        # Check the dataframes for missing rows (times)
        # For now, just print it out so the user is aware of it.
        for exp_id, e_data in Qs_data.items():
            n_missing = e_data['t_start'].isna().sum()
            print(f"{exp_id} is missing  {n_missing} rows == {n_missing/60} minutes")
        print()
    ...
    def assemble_frames(self, period_frames):
        # Generate a multiindex containing the step and experiment time 
        # information. i.e. (limb, discharge, experiment_time_in_seconds)
        experiment_multiindex = self._make_experiment_multiindex()

        Qs_data = {}
        for exp_id, exp_data_frames in period_frames.items():
            exp_data = pd.concat(exp_data_frames)
            exp_data.index.names = ['limb', 'discharge', 't_start', 't_end', 'timestamp']

            assert not exp_data['exp_time'].duplicated().any()
            exp_data.reset_index(inplace=True)
            exp_data.set_index(['limb', 'discharge', 'exp_time','exp_time_hrs'], inplace=True)
            #exp_data.sort_index(level='exp_time', inplace=True)
            exp_data = exp_data.reindex(experiment_multiindex)

            if exp_data['t_start'].isna().any():
                print("Missing ", exp_id, exp_data.index[exp_data['t_start'].isna()][0])

            exp_data.reset_index('exp_time_hrs', inplace=True)
            Qs_data[exp_id] = exp_data

        return Qs_data

    def _make_experiment_multiindex(self):
        # Creates the multiindex for a hypothetical dataframe of the whole 
        # experiment (1 row per second for 8 hrs = 8*3600 rows).
        # Returned multiindex has levels: 'limb', 'discharge' and 'exp_time'
        n_period_rows = 3600
    
        repeated_rows = np.repeat(np.array(self.steps), n_period_rows, axis=0)
        limbs, discharges = repeated_rows.T
        exp_time = np.arange(n_period_rows*8) + 1 # seconds
        exp_time_hrs = exp_time / 3600
        assert len(limbs) == len(exp_time)
        new_multiindex = pd.MultiIndex.from_arrays(
                [limbs, discharges, exp_time, exp_time_hrs],
                names=['limb', 'discharge', 'exp_time', 'exp_time_hrs'],
                )
        return new_multiindex

    def read_period_files(self):
        # Read all the light table txt files (1 per period) and return a 
        # dictionary containing a dataframe for each period {exp_id : {(limb, 
        # discharge, t_start, t_end) : data}}

        period_frames = defaultdict(dict)
        period_t_start_dict = {step : 3600*step_i for step_i, step in enumerate(self.steps)}

        # Read all available files and fill in the empty dataframe
        # Filenames like: lt-results-by-period_1B_rising-62L_t40-t60.txt
        filename_pattern = ("lt-results-by-period"
                "_([1-5][AB])" # Experiment code
                "_(falling|rising)-(\d{2,3})L" # Limb and discharge
                "_t(\d{,2})-t(\d{,2})" # Start and end times
                ".txt"
                )
        filename_parser = re.compile(filename_pattern)
        for qs_filename in os.listdir(settings.lighttable_data_dir):
            m = filename_parser.match(qs_filename)
            if m is None:
                print(f"Unexpected file: {qs_filename}")
                continue
            exp_id, limb, discharge, t_start, t_end = m.groups()
            assert (exp_id in self.experiment_keys
                and (limb, discharge) in self.steps
                and t_start in self.valid_times
                and t_end in self.valid_times), (
                        f"Unexpected filename values: {qs_filename}"
                        )
            filepath = pjoin(settings.lighttable_data_dir, qs_filename)
            #global Qs_column_names
            loading_kwargs = {
                    #'columns' : Qs_column_names,
                    'delimiter' : '\t',
                    }
            period_data = self.load_txt_file(filepath, loading_kwargs)

            t_relative = np.arange(int(t_start) * 60, int(t_end) * 60)
            p_t_start = period_t_start_dict[(limb, discharge)]
            period_data['exp_time'] =  t_relative + p_t_start + 1
            period_data['exp_time_hrs'] =  period_data['exp_time'] / 3600
            period_data['feed'] = self.feed[exp_id][p_t_start//3600]

            period_frames[exp_id][m.groups()[1:]] = period_data

        return period_frames

    def _write_pickle(self, data_object, pickle_path):
        print(f"Writing data object to pickle at {pickle_path}.")
        print(("Note: pickles are okay for temporary personal use. "
            "However they are easy to exploit by nefarious people. "
            "Do not use pickles that you didn't create."))
        try:
            data_object.to_pickle(pickle_path)
        except AttributeError:
            with open(pickle_path, mode='wb') as pkl_file:
                pickle.dump(data_object, pkl_file)

    def _load_pickle(self, pickle_path):
        # Load pickle data from a given filepath and return the object
        print(f"Loading pickle {pickle_path}.")
        print(("Note: pickles are okay for temporary personal use. "
            "However they are easy to exploit by nefarious people. "
            "Do not use pickles that you didn't create."))
        try:
            data_object = pd.read_pickle(pickle_path)

        except AttributeError:
            with open(pickle_path, mode='rb') as pkl_file:
                data_object =  pickle.load(pkl_file)
        return data_object

    def load_xlsx_file(self, filepath, pd_kwargs):
        data = pd.read_excel(filepath, **pd_kwargs)
        return data

    def load_txt_file(self, filepath, kwargs, flip=False):
        # filepath is the filepath of a txt file
    ...
    def clean_Qs_data(self):
        ...
        for exp_code in self.experiment_keys:
        #for exp_code in ['1A']:
    ...
    def analyze_pca_all(self, **kwargs):
        ...
        for i, exp_code in enumerate(sorted(self.Qs_data.keys())):
            data = self.Qs_data[exp_code].copy()
    ...
    def _analyze_pca_experiment(self, exp_code, **kwargs):
        ...
        #experiment = self.omnimanager.experiments[exp_code]
        raw_pd_data = self.Qs_data[exp_code].copy()
    ...
    def prep_raw_data(self, raw_pd_data):
        ...
        raw_pd_data.reset_index(inplace=True)
        extra_cols = {
                ...
                'limb' : raw_pd_data.pop('limb').values,
                't_start' : raw_pd_data.pop('t_start').values,
                't_end' : raw_pd_data.pop('t_end').values,
                }
        ...
        # Get grain sizes 
        extra_cols['grain_sizes'] = np.array([
            float(s.split()[1]) for s in raw_pd_data.columns
            ])
        return extra_cols
    ...
    def _plot_reconstruction_fit_single(self, **kwargs):
        ...
        recon = kwargs['recon']
        fig_name_parts = kwargs['fig_name_parts']
        save_plots = kwargs.get('save_plots', True)
    ...
    def _plot_eigenvectors(self, **kwargs):
        exp_code = kwargs['exp_code']
        grain_sizes = kwargs['grain_sizes']
        evectors = kwargs['evectors']
        keep_modes = kwargs['keep_modes']
        fig_name_parts = kwargs['fig_name_parts']
        save_plots = kwargs.get('save_plots', True)
    ...
    def _plot_PCs(self, **kwargs):
        exp_code = kwargs['exp_code']
        exp_time_hrs = kwargs['exp_time_hrs']
        PCs = kwargs['PCs']
        keep_modes = kwargs['keep_modes']
        fig_name_parts = kwargs['fig_name_parts']
    ...
    def analyze_2pass_stability(self, **kwargs):
        ...
        # Process kwargs
        make_plots = kwargs.get('make_plots', True)
        n_p1_clusters_list = kwargs.get('n_p1_clusters_list', [30])
        n_p2_clusters_list = kwargs.get('n_p2_clusters_list', [5])
    ...
    def _analyze_2pass_stability(self, **kwargs):
        ...
        # Process kwargs
        make_plots = kwargs.get('make_plots', True)
        n_p1_clusters = kwargs.get('n_p1_clusters', 30)
        n_p2_clusters_list = kwargs.get('n_p2_clusters_list', [5])
        show_plots = kwargs.get('show_plots', True)
        subplots_shape = kwargs.get('subplots_shape', None)
...
if __name__ == "__main__":
    ...
    project.load_data_files()

    # Attempt to identify outliers. This way is too hacky...
    # There is probably a more statistically sound method out there.
    project._plot_outliers_prominence()

    # Run PCA on each experiment separately
    project.analyze_pca_individually(make_plots=True, save_output=True)

    # Run PCA on the combined data from all 8 experiments
    project.analyze_pca_all(make_plots=True, save_output=True, save_plots=True)

    # Analyze the pca data for clusters
    project.reload_pca_pickle('all')
    project.analyze_clusters_all()

    # Analyze the pca data again with two passes of clustering
